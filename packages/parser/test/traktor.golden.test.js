...
const fixturePath = join(__dirname, 'fixtures', 'scripts_data', 'TraktorSample Data', 'HISTORY-2020-02-15.txt');
    ...
    it('matches legacy output for a known fixture (key sections)', () => {
        ...
        const unboxNumbersDeep = (value) => {
            ...
            if (!value || typeof value !== 'object')
                return value;
            if (Array.isArray(value))
                return value.map(unboxNumbersDeep);
            return Object.fromEntries(Object.entries(value).map(([k, v]) => [k, unboxNumbersDeep(v)]));
        };
        const normalizeBpmDataForCompare = (report) => {
            const bpmStrings = (report?.master_track_log ?? [])
                .map((r) => r?.BPM)
        ...
        expect(unboxNumbersDeep(normalizeBpmDataForCompare(tsReportNormalized))).toEqual(unboxNumbersDeep(normalizeBpmDataForCompare(legacyReportNormalized)));
